...
作者：Haoyi Duan et al.（MIT + 斯坦福 + CMU，ONR/NSF 资助）

---

### 这篇论文在说什么（一句话）
第一个把 3D 场景生成、4D 生成、视频生成（I2V/T2V）**统一放在同一框架下比较**的 benchmark，核心发现：**3D 方法几何准但没动态，视频方法有动态但控制差**——两类方法各有系统性短板，没有一个能全部搞定。

---

### 核心设计思路

把世界生成分解为连续的"next-scene 生成"任务，每步由三元组定义：
```
(C, N, L)
C = 当前场景（图像 + 文本描述）
N = 下一场景的文本描述
L = 空间布局（camera trajectory + 文字描述）
```

这个设计让 3D/4D/I2V/T2V 四类完全不同的模型能被统一评测——所有模型最终都生成**视频**，用同一套指标比较。
...
|------|------|
| 总测试样本 | **3,000 条** |
| 静态场景（室内/室外） | 2,000 条，10 类场景 |
| 动态场景 | 1,000 条，5 类动态（刚体/流体等） |
| 视觉风格 | 真实感 + 风格化（每例有对应风格化版本） |
| 测试模型数 | **19 个**（5 I2V + 7 T2V + 6 3D + 1 4D） |

---

### 评测指标（3 大方面，共 10 个指标）

**方面 1：Controllability（可控性）**
- **Camera controllability**：相机轨迹误差（旋转误差 × 平移误差）
- **Object controllability**：指定物体是否出现（开放集检测成功率）
- **Content alignment**：CLIPScore 衡量语义对齐

**方面 2：Quality（质量）**
- **3D consistency**：用 DROID-SLAM 估计深度，测几何稳定性
- **Photometric consistency**：外观/纹理跨帧一致性（CLIP/DINO 特征）
- + 其他视觉质量指标

**方面 3：Dynamics（动态性）**
- **Motion accuracy**：动态物体运动是否正确（静止的该静止，运动的该运动）
- + 运动流畅性等指标
...
**发现 1：VBench 无法区分"能生成新场景"的模型**
> *"While VBench rates Models A and B similarly based on single-scene video quality, WorldScore differentiates them by identifying that Model B fails to **generate a new scene** or follow the instructed camera movement."*

VBench 对两个模型打分接近，但一个根本没有按照指令生成新场景——VBench 看不出来，WorldScore 能。

**发现 2：3D 方法 vs 视频方法的系统性 Tradeoff**
> *"For 3D scene generation models, while they excel in static world generation, **extending to 4D representations and incorporating dynamics remains challenging**. For video generation models, the main challenges include **controllability, long-sequence generation, and generating outdoor scenes**."*

| 方法类型 | 优势 | 劣势 |
|---------|------|------|
| 3D 生成 | 几何精确，静态场景好 | 无动态，难扩展到 4D |
| 视频生成 | 有动态，外观真实 | 可控性差，长序列失效，户外场景弱 |

**没有任何一个模型能同时在三个维度都表现好。**

**发现 3：Outdoor scenes 是视频生成的系统性盲区**
视频生成模型在室内场景表现相对好，但在户外（复杂背景、光线变化、远景）上显著变差。

---

### 局限性（论文承认）

WorldScore 测的是**几何/空间控制层面**的世界生成能力——camera trajectory compliance、物体出现与否、3D 几何一致性。它**不测物理推理**，不测因果，不测反事实。本质上仍是"生成质量"的高级版本。

---

### 🔑 我们的 Insight（精读后）

**Insight 1：WorldScore 和 VBVR 测的是完全不同的东西**
WorldScore 的"world" = 空间几何世界（3D scene with camera control）  
VBVR 的"world" = 物理推理世界（causal/physical understanding）  
两者互不重叠，是互补的——WorldScore 是最好的几何/可控性 benchmark，VBVR 是最好的推理能力 benchmark。

**Insight 2：Tradeoff 发现揭示了更深层的问题**
3D 方法和视频方法各有短板，说明**当前没有任何方法能作为真正的 World Simulator**。这个 tradeoff 的存在，进一步证明了专门评测物理推理能力的 VBVR 的独立价值——即使哪天几何生成问题被解决，物理推理的 gap 仍然存在。

**Insight 3："统一框架"思路对 VBVR-2.0 的启发**
WorldScore 把 3D/4D/I2V/T2V 统一在一个框架里，这对 VBVR-2.0 有借鉴意义：VBVR-2.0 可以把不同推理难度层次（几何推理 / 物理推理 / 因果推理 / 反事实推理）放在一个统一的 leaderboard 里，让社区一眼看出不同模型的能力剖面。

**Insight 4：VBench "看不出新场景生成失败"的发现**
这再次证明了 TK-3：VBench 等传统 benchmark 对真正的 world model 能力没有区分度。WorldScore 证明这在几何层面成立，VBVR 证明这在推理层面成立——两个方向的证据共同指向同一结论。
...
作者：Yue Hu, Siyuan Huang et al.（AgiBot + SJTU + CUHK）

---

### 这篇论文在说什么（一句话）
第一个专门针对**具身世界模型**（机器人操控场景）的 benchmark，提出三维评测框架，核心发现：**Domain-adapted（专门微调的）模型比通用商业模型好得多，但所有模型在"精细抓取行为"上仍然失败**——VBench 完全无法区分这些差异。

---

### 核心设计

**评测设定**：给定初始视频帧（指定机器人和场景）+ 操控指令 → EWM 自回归生成后续帧，直到任务完成。

**三个评测维度**：

| 维度 | 测什么 | 关键约束 |
|------|--------|---------|
| **Visual Scene Consistency** | 背景、物体、机器人形态保持静止 | 只有机器人关节/末端执行器应该动 |
| **Motion Correctness** | 末端执行器（EEF）轨迹是否正确 | 用 SHD（Hausdorff距离）+ NDTW（动态时间规整）+ DYN（速度/加速度一致性）精确度量 |
| **Semantic Alignment & Diversity** | 是否理解语言指令 + 泛化到多样任务 | 30个样本，10种任务，含动作顺序约束 |

**数据集**：基于 AgiBot-World（最大的真实机器人操控数据集），30个样本，10类任务，每个样本有 GT 轨迹。

**7 个测试模型**：
- 开源：OpenSora 2.0, LTX, COSMOS-7B
- 商业：Kling-1.6, Hailuo I2V-01-live
- Domain-adapted：LTX_FT, EnerVerse（专门为具身场景微调）

---

### 关键发现

**发现 1：Domain-adapted > Commercial > Open-source（三类模型差距显著）**
> *"Domain-adapted models show the **best overall performance**, particularly in semantic and dynamic dimensions, demonstrating that targeted fine-tuning significantly enhances task understanding and motion alignment."*

即使是最好的通用商业模型（Kling），也显著差于专门微调的具身模型。

**发现 2：所有模型都有"空抓取"行为（fine-grained action grounding 失败）**
> *"they occasionally exhibit **empty grasping behaviors**, revealing limitations in fine-grained action grounding."*

即使 domain-adapted 模型也无法保证末端执行器与目标物体的精细对齐——这是所有模型的共同失败点。

**发现 3：各模型的特征性失败模式**

| 模型 | 特征性失败 |
|------|-----------|
| COSMOS | 无法保持一致视角（相机参数控制差） |
| LTX | 场景突变、任务执行不连贯、经常生成静止状态 |
| OpenSora | 机器人手臂抖动，频繁生成静态视频 |
| Hailuo | 场景卡通化，视觉真实性差 |

**发现 4：VBench 无法区分具身场景的前景/背景**
> *"VBench struggles to **separate foreground and background features**, limiting the effectiveness of its subject-level metrics. In contrast, our Scene Consistency metric, leveraging fine-tuned DINOv2, excels at capturing layout structure."*

这是**第三次**来自不同 benchmark 的 VBench 失效证据（WorldModelBench、WorldScore、EWMBench 均发现此问题）。

---

### 局限性（论文明确承认）

- 目前只追踪末端执行器轨迹，未涉及整个机械臂状态
- 只评测固定视角场景，未来需要动态相机设定
- 只覆盖 manipulation 任务，未扩展到 navigation 和 mobile manipulation

---

### 🔑 我们的 Insight（精读后）

**Insight 1：具身场景中"运动"是最难的维度——对 VBVR-2.0 的具身方向有直接支撑**
EWMBench 证明 motion correctness 是当前所有模型最薄弱的维度。VBVR-2.0 如果加入"给定动作指令，预测末端执行器到达哪里"的推理题，这将是高价值且目前完全空白的方向。

**Insight 2：EWMBench 的"评测生成质量" vs VBVR 的"评测推理能力"——再次确认差异化**
EWMBench 的 Motion Correctness 用 SHD/NDTW/DYN 精确度量轨迹相似度（仍是生成质量）。VBVR 测的是"你理解这个动作的结果吗"（推理）。同一个具身场景，EWMBench 测生成，VBVR 测理解——两者完全互补。

**Insight 3：VBench 失效的第三块证据**
WorldModelBench（物理）+ WorldScore（几何）+ EWMBench（具身前景/背景）= 三个独立实验都证明 VBench 无法区分 world model 能力。这三个证据合在一起，在 VBVR-2.0 paper 里可以形成极强的 motivation 论点。

**Insight 4："Domain-adapted >> General"的发现意味着什么**
专门微调的模型远超通用模型说明：**当前通用大模型的 world model 能力远不够**，fine-tuning 对具身场景是必须的。对 VBVR 来说，这意味着 VBVR 的测试结果不会因为大模型 scaling 而很快被追平——domain gap 是真实存在的障碍。
...
作者：HKUST（Pascale Fung 组）

---

### 这篇论文在说什么（一句话）
第一个从纯视觉观察测试高层世界建模和长时程程序规划的 benchmark，核心数字：**最强模型世界建模只达 57%，规划只达 38%，人类近 100%**，且模型的最佳策略是把视频转成文字来"作弊"。

---

### 两个子 Benchmark

| 子 Benchmark | 测什么 | 任务形式 |
|-------------|--------|---------|
| **WorldPrediction-WM** | 理解动作因果（给定状态转变，识别哪个动作造成的） | 从反事实候选中选正确动作 |
| **WorldPrediction-PP** | 长时程程序规划（给定目标，排列正确动作序列） | 从候选中选有序序列 |

**关键防作弊设计**：提供"action equivalents"（相同动作在不同背景的视频）作为候选，强迫真正的语义理解而非低层视觉线索。

**数据**：5 个大型数据集（COIN / CrossTask / EgoExo4D / EPIC-KITCHEN-100 / IKEA-ASM）。

---

### 关键发现（直接数字）

> *"frontier models barely achieve **57% accuracy on WorldPrediction-WM** and **38% on WorldPrediction-PP** whereas humans are able to solve both tasks perfectly."*

| 测试对象 | WM（世界建模） | PP（过程规划） |
|---------|--------------|--------------|
| 最强前沿模型 | **57%** | **38%** |
| 人类 | ~100% | ~100% |

**最强模型的"作弊"方式**：
> *"Current best-performing models largely rely on **textual descriptions** to tackle both tasks... whereas humans are able to solve these tasks from **observations alone**."*

最好的模型用 Qwen 2.5-VL(72B) 把视频转文字，再用 Llama/DeepSeekR1 做文字推理——模型真实的视觉世界理解能力比 57% 更差。

---

### 🔑 我们的 Insight（精读后）

**Insight 1：WorldPrediction 测"认识因"，VBVR-2.0 测"预测果"——互补**
WorldPrediction-WM：给定状态变化 → 识别原因（discriminative）  
VBVR-2.0 因果任务：给定状态+动作 → 预测结果（generative）  
两者互补，VBVR-2.0 测的是更主动的方向。

**Insight 2：57% / 38% 直接呼应 VBVR 的 68.5% vs 97.4%**
不同 benchmark、不同任务设计，同一结论：模型距人类 30-60% 的系统性差距。

**Insight 3："靠文字作弊"揭示视觉理解的本质缺失**
模型把视频转文字再推理——说明真实视觉世界理解能力比数字更差。VBVR 的视频推理框架测的是无法转文字绕过的真实能力，这是 VBVR 设计的核心价值。

**Insight 4：规划 38% 是最强支撑数字**
多步规划准确率只有 38%，直接支撑 VBVR-2.0 的因果链推理方向（A→B→C 多步），该维度几乎是空白且当前模型极弱。
...
作者：Johns Hopkins + MIT（Yilun Du 组）

---

### 这篇论文在说什么（一句话）
第一个真正的**闭环评测**框架，揭示三个"意外"：**视觉质量不等于任务成功**、Post-training 比升级基础模型更有效、Inference-time scaling 显著提升闭环表现。还发现了第一个 World Model 数据 Scaling Law。

---

### 核心设计

- **统一 Action API**：允许异质 world model 接入同一 agent 规划框架
- **闭环协议**：WM 生成未来帧 → agent 感知/规划/执行 → 再次输入 WM → 循环
- **首要指标**：Task Success Rate，不是 FVD/视觉质量
- **4 个具身评测任务**：多步感知、规划和控制
...
> *(2) **scaling post-training with action-observation data is more effective than upgrading the pretrained video generators**;*  
> *(3) **allocating more inference-time compute allows WMs to substantially improve closed-loop performance**.*

| 意外 | 含义 |
|------|------|
| 视觉质量 ≠ 任务成功 | 可控性比视觉质量更重要 |
| Post-training > 更大基础模型 | 针对性数据比规模更有效 |
| Inference-time scaling 有效 | 推理时加算力显著提升闭环表现 |

**数据**：*"Our experiments reveal **large gaps between visual metrics and task success**"*  
**Bonus**：发现了 world model 领域**第一个数据 Scaling Law**（post-training 数据量 vs 任务成功率幂律关系）

---

### 🔑 我们的 Insight（精读后）

**Insight 1："可控性 > 视觉质量"颠覆整个领域认知，和 ACT-Bench 完全一致**
两个独立 benchmark（World-in-World + ACT-Bench）从不同角度证明：可控性才是 world model 核心，视觉质量是误导性指标。

**Insight 2：VBVR 的"准闭环"是最好的轻量折中**
真闭环极复杂（仿真环境 + agent + 多步执行）。VBVR 的 first frame → video → 推理问题，不需要 agent 真实执行，但测的是闭环所需的核心认知能力。

**Insight 3：Post-training 数据 >> 升级基础模型——VBVR-2.0 是天然的 post-training 数据源**
VBVR-2.0 程序化 GT = verifiable reward = RLVR post-training 信号。用 VBVR 数据做 post-training 可以提升 world model 的闭环表现，大幅提升 VBVR-2.0 的生态价值。

**Insight 4：Inference-time scaling 有效——配合 SWIFT（Paper 22）形成闭环**
两篇共同指向：VBVR-2.0 的程序化 GT 是 test-time verifier 天然来源，能支撑 inference-time scaling 研究。
...
作者：Toyota Research Institute + MIT

---

### 这篇论文在说什么（一句话）
自动驾驶 world model 的评测从来不测"动作 → 结果"，只测视频好不好看。ACT-Bench 设计了专门的动作可控性测试，发现：**给 Vista 下"向左转"指令，生成的视频和"向右转"几乎一样**——最先进的驾驶 world model 根本没有理解动作。

---

### 核心设计

- **数据集**：nuScenes 数据集中的短驾驶场景，标注高层动作（"curving to right/left"等）和对应 GT 轨迹
- **ACT-Estimator**：从生成视频中估计轨迹的运动估计模型，与 GT 轨迹比较
- **指标**：动作-轨迹一致性（指令与执行的偏差）+ 轨迹空间精度
- **对比基线**：Vista（当时 SOTA）vs Terra（本文提出的新模型）

---

### 关键发现（直接引用 + 图示）

> *"Vista fails to follow the instruction [to curve left]... the lower example illustrates that **Vista fails to follow the instruction**."*

**最核心发现**：
- Vista（当时最强驾驶 WM）对不同动作指令生成几乎相同的视频
- 视觉质量（FVD）高 ≠ 动作可控性高，两者几乎**零相关**
- Terra（本文新模型）在动作可控性上显著优于 Vista

---

### 🔑 我们的 Insight（精读后）

**Insight 1："动作指令形同虚设"是最震撼的 world model 能力缺陷**
模型生成的视频质量可以很高，但它完全忽略了动作输入——这意味着"action-conditioned world model"在很大程度上是名不副实的。VBVR-2.0 的 action-conditional 推理方向直接针对这个缺陷。

**Insight 2：和 World-in-World（Paper 8）的"可控性 > 视觉质量"完全一致**
两篇独立实验，同一结论。ACT-Bench 在驾驶场景，World-in-World 在具身场景——跨领域的一致发现说明这是 world model 的系统性问题，不是某个特定模型的问题。

**Insight 3：VBVR 的"准闭环"推理框架天然能测 action-conditional 理解**
VBVR 的任务形式（first frame + action → video → question）只需要在现有框架上加入 action 条件，就能测试"模型是否真正理解了动作的物理后果"。这是程序化数据引擎的直接延伸，实现成本低，测试价值高。

**Insight 4：开放 benchmark 的重要性**
ACT-Bench 的动机之一是现有评测（Vista/GenAD）用闭源模型，不可复现。VBVR 完全开放（程序化生成 + 公开 leaderboard），这是重要的社区贡献点。
...
### 这篇论文在说什么（一句话）
系统梳理视频生成模型物理认知能力的演进，提出三层进化框架（感知→被动认知→主动认知），核心结论：**"visually realistic yet physically absurd"是当前所有模型的系统性问题，动力学层（力/质量/能量）几乎完全未解决**。

---

### 三层物理认知进化框架

> *"schematic perception, passive learning of physical knowledge, and active interaction with the environment"*

| 层级 | 描述 | 代表技术 | 当前状态 |
|------|------|---------|---------|
| **Layer 1: Basic Schematic Perception** | 低保真视觉模式的单向刺激，如物体定位 | 早期 GAN，运动模式 | ✅ 基本解决 |
| **Layer 2: Passive Cognition** | 从物理仿真器或 LLM 嵌入预存的静态物理知识 | physics-informed diffusion | ⚠️ 部分解决 |
| **Layer 3: Active Cognition** | 与环境主动交互，实现物理可信的未来预测 | world simulators | ❌ 几乎未解决 |
...
**发现 1：三个物理层次的进展极不均衡**
- 感知层（光影/纹理/外观）：高分辨率模型已基本解决
- 运动层（速度/轨迹/碰撞）：大模型后有改善，但细节错误仍频繁
- **动力学层（Newtonian dynamics / momentum / energy conservation）：几乎完全未解决**

**发现 2：Scaling 不能自动带来物理认知**
训练数据增大、模型参数增大 → 感知层提升明显 → 但动力学层几乎没有进步。物理理解需要专门设计，不是 scaling 的自然涌现。

**发现 3：现有评测几乎不覆盖动力学层**
大部分 benchmark（FVD/VBench 等）测感知层，少数（WorldModelBench）测运动层边缘，**动力学层几乎无任何 benchmark 系统覆盖**。

---

### 🔑 我们的 Insight（精读后）

**Insight 1：动力学层 = VBVR-2.0 最有价值的物理推理切入点**
感知层已有很多 benchmark，运动层 WorldModelBench 在做，动力学层是真空。VBVR-2.0 如果测"Newton 第二定律 / 动量守恒 / 能量守恒"级别的推理，是当前最空白也最有价值的方向。

**Insight 2："Active Cognition"层 = 主动与环境交互 → 对应 VBVR 的推理任务**
论文的最高层（Active Cognition）要求模型主动与环境交互来做预测。VBVR 的推理任务形式上正好对应这一层：不是被动接受视频，而是主动"回答关于世界状态的问题"。

**Insight 3：Scaling 不解决物理认知 → 需要专门的 benchmark 和训练信号**
这个结论直接说明：即使 GPT-5 来了，物理认知能力也不会自动突破。需要 VBVR-2.0 这样的专门 benchmark 来提供正确的评测信号和训练数据。
...
### 这篇论文在说什么（一句话）
现有 world model 评测完全忽略了空间记忆一致性——模型走过一个地方，再绕回来，已经"忘记"了之前看到的场景。LoopNav 提出循环导航任务来测试这个能力，发现**当前模型是"视觉梦机器"：看起来好看但在空间记忆上完全不可靠**。

---

### 核心设计

- **数据集**：Minecraft 环境中 **250 小时（2000 万帧）** 的循环导航视频，含对应动作
- **任务**：A → B → C → A 式的循环轨迹，模型需要在返回出发点时精确重建出发时的场景
- **评测指标**：循环轨迹中返回点的重建质量（而非整体生成质量）
- **课程设计**：从短轨迹到长轨迹递增，逐步增加记忆负担
- **4 个基线模型**：DIAMOND 等主流 world model

---

### 关键发现

**发现 1：当前模型是"dream machines"，不是可靠的 world model**
> *"world models trained on such data tend to rely on **global environmental priors to hallucinate plausible next frames**, rather than leveraging episodic memory to reconstruct previously observed content... leading to models that function more as visually impressive but impractical 'dream machines'"*

模型生成的下一帧"看起来合理"，但不是真的"记住了"——它在用全局先验幻觉出来的，不是从记忆中检索的。

**发现 2：主流 Transformer 上下文长度是硬瓶颈**
> *"mainstream Transformer-based networks are typically limited to memorizing only a few or a dozen past frames... real-world exploration trajectories often span hundreds or thousands of frames"*

注意力机制的二次复杂度导致上下文长度极其有限，而真实导航需要跨越数百甚至数千帧的记忆。

**发现 3：定量指标不足，需要定性评估**
> *"qualitative evaluation plays an equally critical role... Human visual inspection remains the most straightforward and effective way"*

没有单一量化指标能完全捕捉空间一致性——说明这个评测维度本身也是难题。

---

### 🔑 我们的 Insight（精读后）

**Insight 1："Dream Machine" 是对当前 WM 最精准的描述**
"视觉好看但空间记忆不可靠"——这个描述不仅适用于空间导航，也适用于 VBVR 测的推理维度。模型在用"看起来合理"替代"真正理解"。

**Insight 2：VBVR-2.0 的记忆测试方向完全可行**
视频前半段出现信息 → 视频后半段问相关推理问题。这不需要 Minecraft 环境，用程序化场景就能生成，且 GT 完全精确。LoopNav 验证了这个方向的重要性，VBVR 的程序化引擎可以更优雅地实现它。

**Insight 3：250小时数据 vs 程序化无限生成**
LoopNav 需要收集 250 小时真实 Minecraft 数据。VBVR 的程序化引擎可以无限生成各类记忆测试场景，成本接近零。
...
### 这篇论文在说什么（一句话）
15 个最强 VLM（o3/GPT-4o/Gemini 等），23 个维度，660 次实验——结论：**几乎所有模型在区分运动轨迹时接近随机，某些模型认为"蓝色物体比绿色物体快"**，VLM 的 world model 能力远比看起来差。

---

### 两阶段原子级框架（基于认知科学）

**阶段 1：感知**：视觉 / 空间 / 时间 / 定量 / **运动**（最难）  
**阶段 2：预测**：**机械仿真** / **传递推理**（A→B→C）/ **组合推理**（新组合泛化）

规模：23 维度 × 6 仿真环境 × 660 实验 × **15 个前沿 VLM**

---

### 关键发现

> *"almost all models perform at **near-random accuracy** when distinguishing motion trajectories."*  
> *"some models tend to believe **blue objects move faster than green ones**."*  
> *"Even the most advanced models achieve **less than 60% accuracy** in spatial positioning."*  
> *"VLMs excel in scenarios with pronounced differences, they **struggle with 3D and dynamic perception**, fail to differentiate subtle physical distinctions."*
...
**Insight 1：运动轨迹识别接近随机 → VBVR-2.0 运动推理维度高价值**
o3 在区分基础运动轨迹上接近随机，VBVR-2.0 的 action-conditional 推理任务会有巨大模型分化空间。

**Insight 2：23 维度 vs VBVR 5 维度——互补的能力图谱**
WM-ABench 覆盖感知层（视觉/空间/时间/定量/运动），VBVR 覆盖推理层（知识/抽象/空间/变换/感知）。VBVR-2.0 扩展到运动感知维度，正好填补两者的交集。

**Insight 3："蓝色=快"是物理属性解耦失败的经典案例**
VBVR-2.0 可专门设计"属性解耦"推理题——控制颜色与物理属性的独立关系，直接测试这个 disentanglement 能力。

**Insight 4：传递推理失败 → 直接支撑因果链方向**
A→B→C 传递推理在所有 VLM 上系统性失败，这正是 VBVR-2.0 因果链推理方向的核心。
...
### 这篇论文在说什么（一句话）
提出 WorldTest 框架和 AutumnBench（43 环境 129 任务），用 517 名人类 vs Claude 4 Sonnet / Gemini 2.5 Pro / o3 对比——结果：**人类在所有环境和任务类型上都超过模型，人类接近满分而模型频繁失败**，且 scaling compute 只在部分环境有效。

---

### WorldTest 框架设计

**核心创新**：把 world model 评测分成两个独立阶段：
1. **无奖励探索阶段**：agent 自由与环境交互，收集信息，建立 world model
2. **测试阶段**：在不同但相关的环境中执行 3 类任务（不依赖固定奖励）

**为什么重要**：现有评测把"探索"和"测试"混在一起，或者只用 next-frame prediction 作为代理指标——都不能真正测 world model 的能力。

**AutumnBench**：
- 43 个格子世界环境
- 129 个任务，3 类：
  - **Masked-frame prediction**（预测被遮住的帧）
  - **Planning**（规划动作序列）
  - **Causal change detection**（检测世界规则何时改变）

---

### 关键发现（直接数字）

**测试对象**：517 名人类 vs Claude 4 Sonnet / Gemini 2.5 Pro / o3

> *"humans outperform the models **across all environments and task types**"*  
> *"humans achieve **near-optimal scores** while existing models **frequently fail**"*

**人类的独特行为**：
> *"human participants used the **reset action far more often** than the models... suggesting a possible link between humans' use of resets as an **exploration or hypothesis-testing mechanism** and their ability to refine internal models of the world"*

人类主动用"reset"来验证假设——这是一种主动假设检验行为，而模型没有这种行为。

**Scaling 的局限**：
> *"scaling compute improves performance **only in some environments but not others**"*

Compute scaling 不是万能的——对某些 world model 任务无效。

---

### 🔑 我们的 Insight（精读后）

**Insight 1："Causal change detection"是最重要的新方向**
检测世界规则何时改变（前半段规则 A，后半段突然变规则 B）——这个任务类型在所有现有 benchmark 中都没有，是最新的方向。VBVR-2.0 完全可以实现：程序化生成视频，前半段正常物理，后半段改变某个物理参数（如重力方向反转），问模型"世界规则改变了吗？改变了什么？"

**Insight 2："Reset 作为假设检验"揭示了人类和 AI 的根本差异**
人类用 reset 来主动验证对世界的理解；模型没有这种主动探索行为。这说明当前 AI world model 是"被动接受"而不是"主动建模"——这正是 VBVR 的 active cognition 方向要解决的。

**Insight 3：517 名人类参与者 → 可信的人类基准线**
WorldTest 有大规模人类基准，和 VBVR 的人类 97.4% 形成呼应——两个完全不同的研究团队都证明人类对 world model 任务的表现接近完美，而模型远未达到。

**Insight 4：Scaling compute 在某些任务无效 → VBVR-2.0 的任务设计价值**
如果 scaling 无法自动提升 world model 能力，那么专门设计的 benchmark 和针对性训练数据（如 VBVR-2.0 的程序化 GT）才是关键。这强化了 VBVR-2.0 的独立价值。
...
> 随每篇论文精读后更新，记录当前已确认的核心结论

---

## TK-1：两个根本判断已被社区共识确认
**来源：Paper 1（清华 Survey）+ Paper 2（GigaAI Survey）**

两篇独立来源的 Survey 得出同一结论：
> "视频生成 ≠ 世界模型"（Paper 2 原话）  
> "case-based 而非 rule-based"（Paper 1 原话）

模型学的是"训练分布里的样子"，不是"物理规律"。这是整个 benchmark 领域的根本动机。

---

## TK-2：Pearl 三层框架是最强的定位语言
**来源：Paper 1 + Paper 2**

```
第一层：关联/Seeing    → 当前所有视频生成模型
第二层：干预/Doing     → action-conditional WM（部分工作）
第三层：反事实/Imagining → 真正的 World Model 应达到此层
```

VBVR-1.0 在第一到第二层之间。**VBVR-2.0 的使命 = 测第三层能力**。直接可用于 paper introduction。

---

## TK-3：三个数字可直接放进 VBVR-2.0 Introduction
**来源：Paper 3 WorldModelBench + VBVR 官网**

| 数字 | 来源 | 含义 |
|------|------|------|
| **0.28** | WorldModelBench | VBench 总分 vs 物理合规的相关性 → 主流 benchmark 对物理理解无预测力 |
| **61%** | WorldModelBench | 最好模型（kling）任务完成率 → 离理想水平差 40% |
| **97.4% vs 68.5%** | VBVR | 人类 vs 最强视频模型推理 gap |

三个数字共同指向：当前模型与人类差距巨大，且现有评测没有正确衡量它。

---

## TK-4：两条评测路线的哲学差异
**来源：Paper 3 WorldModelBench + VBVR**

| 路线 | 代表 | 测的是 | 局限 |
|------|------|--------|------|
| **被动违规检测** | WorldModelBench | 视频有没有违反物理？ | 需大量人工标注；只测"有没有错" |
| **主动推理测试** | VBVR | 给你这个场景，结果是什么？ | 程序化生成，GT精确，无需人工标注 |

**VBVR 层次更高**：被动检测是"找错"，主动推理是"预测"——后者才真正测试物理规律是否被内化。

---

## TK-5：S0-S3 层级是最好的位置描述框架
**来源：Paper 4 WorldSimBench**

```
S0: 文本预测（LLM）
S1: 图像预测
S2: 视频预测（传统 FVD benchmark 在这里）
S3: 可执行视频（World Simulator）← VBVR 在评测此层所需能力
```

VBVR-2.0 定位语言：*"评测 S3 World Simulator 所需的因果推理和物理理解能力"*。

---

## TK-6：程序化 GT 是 VBVR 最根本的工程优势
**来源：对比 Paper 3、4 的人工标注成本**

| Benchmark | GT 来源 | 成本 | 可扩展性 |
|-----------|---------|------|---------|
| WorldModelBench | 67K 人工标注 | 极高 | 差 |
| WorldSimBench | 35,701 条人工反馈 | 极高 | 差 |
| **VBVR** | 程序化仿真引擎 | **接近零** | **无限** |

延伸：VBVR 的程序化 GT 天然是 RLVR 的 verifiable reward 来源（Paper 18 RLVR-World 明确指出这个需求）。

---

## TK-7：GPT-4o 作为视频评测器不可靠
**来源：Paper 4 WorldSimBench**

WorldSimBench 实验：GPT-4o 零样本下评测某些场景的视频质量出现**负相关**。  
→ LLM-as-judge 在视频世界模型评测中会产生误导性结论。  
→ VBVR 程序化 GT 完全绕开这个问题，是最可靠的评测设计。

---

## TK-8：还有 3 篇新 benchmark 待补充（Paper 1 §6.3 指出）

- **T2VPhysBench**：12 条物理定律的系统测试
- **Physics-IQ**：5 个物理领域（固/液/光/热/磁）
- **VBench-2.0**：加入物理和常识维度的升级版 VBench

这三篇在读完 Group 2 后需单独深读。

---

## 当前已确定的 VBVR-2.0 方向支撑

| VBVR-2.0 方向 | 支撑 Paper | 确定程度 |
|--------------|-----------|---------|
| 反事实推理 | Paper 1 §6.1, Paper 2 §5.1.1 | 🔴 最高 |
| 多步因果链 | Paper 1, 2 | 🟠 高 |
| 动力学物理推理 | Paper 3（5类违规只到表层） | 🟠 高 |
| 程序化 GT 引擎 | Paper 3, 4（对比人工标注成本） | ✅ 已确定 |
| Pearl 三层框架 | Paper 1, 2 | ✅ 已确定 |
| S0-S3 定位语言 | Paper 4 | ✅ 已确定 |

---
## TK-9：WorldScore 揭示的系统性 Tradeoff
**来源：Paper 5 WorldScore**

3D 方法和视频方法各有系统性短板：
- 3D 方法：几何精确，但无动态，难扩展到 4D
- 视频方法：有动态，但可控性差，长序列失效

**没有任何模型能同时在 controllability / quality / dynamics 三个维度都表现好。**  
→ 这意味着即使几何生成问题被解决，物理推理的 gap 仍然存在，VBVR 的独立价值不受影响。

---

## TK-10："WorldScore 的 World" ≠ "VBVR 的 World"
**来源：Paper 5 WorldScore**

| | WorldScore | VBVR |
|--|-----------|------|
| "World"的含义 | 空间几何世界（3D + camera trajectory） | 物理推理世界（causal/physical understanding） |
| 测什么 | 几何一致性、可控性、动态性 | 推理正确性 |
| GT 来源 | 真实场景图像 + SLAM | 程序化仿真 |

两者完全互补，不重叠。WorldScore 是几何/空间领域最好的，VBVR 是推理能力领域最好的。

---

## TK-11：VBench 的失效在几何层面也被证明
**来源：Paper 5 WorldScore（Figure 1）**

VBench 对两个模型评分接近，但 WorldScore 发现其中一个根本没有生成新场景。  
结论：**VBench 的失效不只在物理层面（TK-3 已证），在几何/场景控制层面同样失效。**  
→ 传统 benchmark 的系统性失效现在有来自两个独立 benchmark（WorldModelBench + WorldScore）的双重证据。

---

## TK-12：VBench 失效现在有三块独立证据
**来源：Paper 3 WorldModelBench + Paper 5 WorldScore + Paper 6 EWMBench**

| 证据来源 | VBench 在哪里失效 | 具体表现 |
|---------|----------------|---------|
| WorldModelBench | 物理合规层面 | 物理合规 vs VBench 相关性只有 0.28 |
| WorldScore | 几何/场景控制层面 | 无法区分"能不能生成新场景" |
| EWMBench | 具身前景/背景层面 | 无法区分前景运动和背景变化 |

三个独立实验，三个不同维度，共同结论：**VBench 系统性失效，无法评测 World Model 能力**。这是 VBVR-2.0 Introduction 最强的论据群。

---

## TK-13：Domain Fine-tuning >> General Models in Embodied Scenes
**来源：Paper 6 EWMBench**

> *"Domain-adapted models show the best overall performance... targeted fine-tuning significantly enhances task understanding and motion alignment."*

通用模型（Kling/Hailuo/COSMOS）vs 专门微调模型（EnerVerse/LTX_FT）差距显著。  
**含义**：当前通用大模型的具身 world model 能力远不够，Scaling 不能自动解决 domain gap。  
**对 VBVR**：VBVR 的 benchmark gap 不会因为 LLM 规模增大而很快消失——针对性训练才是关键。

---

## TK-14：所有模型在"精细行为 grounding"上共同失败
**来源：Paper 6 EWMBench**

> *"they occasionally exhibit **empty grasping behaviors**, revealing limitations in fine-grained action grounding."*

即使最好的 domain-adapted 模型也无法保证末端执行器与目标物体的精细对齐。  
**含义**：细粒度的物理交互理解是当前所有模型的共同极限，这是 VBVR-2.0 物理推理维度的直接动机。

---

## TK-15：关键数字矩阵——人类 vs 模型的系统性差距
**来源：Paper 7 WorldPrediction + VBVR**

| Benchmark | 任务类型 | 最强模型 | 人类 | Gap |
|-----------|---------|---------|------|-----|
| VBVR | 视频推理（5维度） | 68.5% | 97.4% | **28.9%** |
| WorldPrediction-WM | 高层世界建模 | 57% | ~100% | **43%** |
| WorldPrediction-PP | 长时程过程规划 | 38% | ~100% | **62%** |

三组数字共同证明：视觉世界理解能力是系统性的、跨任务的、巨大的差距——不是某个 benchmark 的偶然结果。

---

## TK-16："可控性 > 视觉质量"被两个独立实验证实
**来源：Paper 8 World-in-World + Paper 9 ACT-Bench**

- World-in-World（具身场景）：*"controllability matters more than visual quality"*
- ACT-Bench（驾驶场景）：Vista 视觉质量高但对动作指令形同虚设，动作可控性与 FVD 几乎零相关

**不同领域、不同 benchmark、同一结论**：可控性才是 world model 的核心，而非视觉质量。这是整个领域的认知颠覆，是 VBVR-2.0 action-conditional 方向的最强支撑。

---

## TK-17：Post-training 数据 >> 升级基础模型
**来源：Paper 8 World-in-World**

> *"scaling post-training with action-observation data is **more effective than upgrading the pretrained video generators**"*

含义：VBVR-2.0 的程序化 GT 不仅是 benchmark，还是 post-training 的数据来源（verifiable reward for RLVR）。这让 VBVR-2.0 的价值从"评测工具"升级到"训练基础设施"。

---

## TK-18："作弊"揭示视觉理解的本质缺失
**来源：Paper 7 WorldPrediction**

最强模型的最佳策略：把视频转文字再用 LLM 推理。  
含义：模型的真实视觉世界理解能力比报告数字更差。VBVR 的视频推理框架（无法用文字绕过）测的是真实能力。

---

## TK-19：物理认知三层框架 —— 动力学层是最大空白
**来源：Paper 10 Physics Cognition Survey**

| 层级 | 代表能力 | 当前状态 |
|------|---------|---------|
| Schematic Perception | 外观/光影/纹理 | ✅ 基本解决 |
| Passive Cognition | 运动模式/轨迹 | ⚠️ 部分解决 |
| **Active Cognition / Dynamics** | Newton定律/动量/能量守恒 | ❌ **几乎完全未解决** |

动力学层不仅无人解决，也无人评测。VBVR-2.0 物理推理方向应聚焦此层。

---

## TK-20：当前 WM 是"Dream Machine"，不是可靠模拟器
**来源：Paper 11 LoopNav**

> *"models rely on global environmental priors to hallucinate plausible next frames, rather than leveraging episodic memory... visually impressive but impractical 'dream machines'"*

模型在用"看起来合理"替代"真正记住"——这个描述适用于所有 world model 任务，不只是空间导航。VBVR-2.0 的推理任务直接暴露这个问题。

---

## TK-21：VLM 区分运动轨迹接近随机，某些模型以为"蓝色=快"
**来源：Paper 12 WM-ABench（660 实验，15 个前沿 VLM）**

- 运动轨迹区分：**接近随机准确率**
- 3D 空间定位：**最强模型 < 60%**
- 物理属性解耦：部分模型认为"蓝色物体比绿色物体快"
- 传递推理和组合推理：系统性失败

**含义**：o3 这种最强推理模型在基础物理感知上也极弱，VBVR-2.0 在这些维度上会有巨大的模型差异化空间。

---

## TK-22："Causal Change Detection"是最新、最空白的任务类型
**来源：Paper 13 Benchmarking WML**

检测世界规则何时改变（规则 A → 规则 B 突然切换）：
- 人类：接近满分
- Claude 4 Sonnet / Gemini 2.5 Pro / o3：**频繁失败**
- Scaling compute 在此任务无效

VBVR-2.0 可实现：程序化生成"物理参数突变"视频（如重力突然反转），测试模型能否识别规则改变。这是最新的、完全未被其他 benchmark 测试的方向。

---

## TK-23：人类主动"假设检验"，模型被动接受
**来源：Paper 13 Benchmarking WML**

> *"humans used the reset action far more often... as an exploration or hypothesis-testing mechanism"*

人类主动重置来验证对世界的理解；模型完全没有这种主动探索行为。这揭示了 AI world model 的根本缺陷：被动拟合而非主动建模。这是 Pearl "三层智能"（TK-2）在实验层面的直接证据。

---

*最后更新：Paper 13 读完后（Paper 1-13 共 13 篇，Group 3 全部完成）*
